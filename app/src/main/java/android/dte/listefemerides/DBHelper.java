...
public class DBHelper extends SQLiteOpenHelper {
	...
	public void createDataBase() throws IOException {
		...
		if (dbExist) {
			...
			// de nuestra aplicacion por lo que podremos sobreescribirla con
	...
	private boolean checkDataBase() {
		...
		try {
			...
			if(android.os.Build.VERSION.SDK_INT >= 4.2){
				myPath = DB_PATH = myContext.getApplicationInfo().dataDir + "/databases/" + DB_NAME;
			} else {
				myPath = DB_PATH = "/data/data/" + myContext.getPackageName() + "/databases/" + DB_NAME;
			}
	...
	public EfemerideItem[] getEfemerides() {
		...
		if(myDataBase.isReadOnly()) {
			Log.d("EXISTEEEEE", "EXISTEEEEE");
		}else{
			Log.d("EXISTEEEEE", "NOOOEXISTEEEEE");
		}

		Log.d("VERSION","Database is there with version: "+myDataBase.getVersion());


		Cursor cc = myDataBase.rawQuery("SELECT * FROM " + TBL_EFEMERIDE
				, null);
